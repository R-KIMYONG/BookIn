...
export const fetchCommentBooks = async ({ page, pageSize, sort, search, searchField }: FetchCommentBooksProps) => {
  ...
  const res = await fetch(`/api/mybooks/comments?${params.toString()}`);
